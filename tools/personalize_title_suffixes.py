"""Body-evidenced title suffixes for site12; does not regenerate manuscripts."""
...
BASELINE_COMMIT = "bc84eec09bd73bcb5bc483cff1a3b7831b8a9f6e"
...
def learning_blocks(source, kind):
    ...
    for block in parser.blocks:
        ...
        if classes & {"center-info", "center-card", "school-list", "review-card", "center-facts-panel", "academy-side-card", "school-card", "subject-info-card", "grade-source-facts", "grade-faq-list", "subject-center-card", "subject-fact-grid", "subject-scenarios", "subject-scenario-card", "subject-checklist"}:
            continue
        ...
        weight = 0
        if kind == "subject":
            if "subject-content-section" in classes:
                # Site12's fourteen newer categories place the actual copy in
                # standalone sections, not a manuscript wrapper. Testimonials,
                # checklists and factual center cards were excluded above.
                weight = (8 if "subject-quick-answer" in classes else 4) if block["tag"] == "p" else 2
            elif classes & {"manuscript-section", "manuscript-article", "grade-main-article", "subject-manuscript"}:
        ...
        if weight:
            ...
            for sentence in re.split(r"(?<=[.!?])\s+", text):
                if len(sentence) >= 12 and not OPERATIONAL.search(sentence):
                    # Some templates put individual difficulties in later cards,
